...
void ShowRoom::drawModernPodium(float cx, float cz, int id) {
	...
	float ringLocalY = 5.0f; // ارتفاع الحلقة عن الأرض

	glDisable(GL_LIGHTING);
	glColor3f(0.0f, 0.4f, 0.8f);

	// تصغير نصف القطر لتصغير حجم الحلقة
	float blueRingRadius = baseR - 50.0f; // نصف القطر أصغر بـ 30 وحدة تقريباً

	cycl.draw(
		Point(cx, ringLocalY, cz),
		blueRingRadius,  // نصف القطر x
		blueRingRadius,  // نصف القطر z
		0.25f,           // سماكة الحلقة (ارتفاع بسيط)
		40,
		5
	);
...
void ShowRoom::drawPodiums() {
	...
	for (int row = 0; row < 3; row++) {
		...
		drawCeilingLightRing(Point(sideX, totalH + 90.0f, posZ));
...
void ShowRoom::drawBuildingBase() {
	...
	// الأرضية الوسطى (سقف الطابق الأول / أرضية الطابق الثاني)
	// الآن تغطي كل المساحة بين الجدران
	glColor3f(0.08f, 0.08f, 0.08f);
	Cuboid(Point(0, floorHeight - 3, 0), t, totalL, totalW).draw();

	// السقف النهائي للطابق الثاني
	glColor3f(0.1f, 0.1f, 0.1f);
	Cuboid(Point(0, totalBuildingH - 3, 0), t, totalL, totalW).draw();

	// الجدران (الارتفاع الكلي للطابقين)
	...
	// الجدار الأمامي والخلفي للطابقين
	Cuboid(Point(totalW / 2, -3, 0), totalBuildingH, totalL, t).draw();  // Right
	Cuboid(Point(-totalW / 2, -3, 0), totalBuildingH, totalL, t).draw(); // Left

	// قطعة فوق الباب في الطابق الأرضي
	Cuboid(Point(0, 100, -totalL / 2), floorHeight - 100, 5, 120).draw();

	// قطعة فوق الباب في الطابق العلوي
